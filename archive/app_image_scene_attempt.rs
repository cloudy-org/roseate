...
use egui::{Color32, Context, CornerRadius, Event, Frame, Margin, Modifiers, Rect, Response, Style, UiBuilder, Vec2};
        ...
        egui::CentralPanel::default()
            ...
            match (&self.image_handler.image.clone(), self.image_handler.image_loaded) {
                ...
                (Some(image), true) => {
                    ...
                    // let scene_size = self.image_scene_size.get_or_insert(image_size);
                    // let scene_size = image_size;

                    let scene_size = ui.input(|i| i.screen_rect().size());
                    ...
                    let center_position = available_rect.min + (available_rect.size() - scene_size) * 0.5;

                    // Now we need to create a rect based off the center position 
                    // for the new isolated UI that we'll be drawing the scene inside.
                    let max_rect = Rect::from_min_size(center_position, scene_size);

                    let image_scene_response = ui.scope_builder(
                        UiBuilder::default()
                            .max_rect(max_rect)
                            .layout(*ui.layout()), 
                        |ui| {
                            egui::Scene::default()
                                .zoom_range(0.001..=1.0)
                                .show(ui, &mut self.image_scene_rect, |ui| {
                    ...
                    let image_scene_size = Vec2::new(
                        initial_size.x / current_size.x * initial_size.x,
                        initial_size.y / current_size.y * initial_size.y
                    ).round();

                    // As we're not in control of the zoom and pan logic we need to calculate
                    // the zoom factor of the image ourselves from the limited information 
                    // we can get from the Scene widget.
                    self.image_scene_zoom_factor = (
                        image_scene_size.x / initial_size.x + 
                        image_scene_size.y / initial_size.y
                    ) * 0.5;

                    self.image_scene_size = Some(image_scene_size);
