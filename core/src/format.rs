...
pub fn determine_image_format_and_size_from_header(path: &PathBuf) -> Result<(ImageFormat, ImageSize)> {
    ...
    let number_of_bytes_read = File::open(path)
        .map_err(
            |error| Error::ImageHeaderReadFailure { 
    ...
    let image_size_image_type = imagesize::image_type(&buffer[..number_of_bytes_read])
    ...
    Ok((image_format, (image_size.width as u32, image_size.height as u32)))
}

pub fn determine_svg_size(path: &PathBuf) -> ImageSize {
    let metadata = svg_metadata::Metadata::parse_file(&path).expect(
